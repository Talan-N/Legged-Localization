#!/usr/bin/env python3
"""
Version 12: V11T plus SmoothL1/Huber velocity loss and circle-focused weighting for Go2.
...
Compared with 10_train_1d_cnn_vxy_terrain_gated.py:
  ...
  7b. Adds optional SmoothL1/Huber velocity loss and a circle-loss multiplier.
      This is intended for runs such as whiteboard circle where trajectory radius
      can remain under-corrected even when average velocity loss improves.
...
def apply_maneuver_loss_multiplier(data: WindowedData, maneuver_name: str, multiplier: float) -> WindowedData:
    """Multiply sample weights for one maneuver class.

    Use this when a specific maneuver, such as circle, is the main evaluation
    concern. The weighted_mean loss normalizes by the sum of weights, so this
    changes relative emphasis without changing the global loss scale much.
    """
    if multiplier <= 0:
        raise ValueError("multiplier must be positive")
    if multiplier == 1.0 or "maneuver_name" not in data.meta.columns:
        return data
    ...
    mask = out.meta["maneuver_name"].astype(str).str.lower() == str(maneuver_name).lower()
    out.sample_weight[mask.values] *= float(multiplier)
    # Normalize to keep average batch loss magnitude comparable across runs.
    out.sample_weight = (out.sample_weight / max(1e-8, float(np.mean(out.sample_weight)))).astype(np.float32)
    out.meta["sample_weight"] = out.sample_weight
    out.meta["maneuver_loss_multiplier"] = np.where(mask.values, float(multiplier), 1.0)
    return out
...
class LossConfig:
    ...
    velocity_loss_type: str = "smoothl1"
    residual_loss_type: str = "smoothl1"
...
def compute_loss(model, batch, config: LossConfig, device: str) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    # Train the quantity we actually care about at velocity level, normalized
    # by residual scale so vx and vy have comparable influence.
    #
    # V12 change: SmoothL1/Huber is the default. It behaves like MSE for small
    # errors but like L1 for large slip/outlier errors, which prevents the model
    # from over-chasing isolated spikes at the expense of trajectory shape.
    velocity_error_scaled = (corrected_vxy - truth_batch[:, :2]) / y_scale
    if config.velocity_loss_type == "mse":
        velocity_per_axis = velocity_error_scaled ** 2
    elif config.velocity_loss_type == "smoothl1":
        velocity_per_axis = F.smooth_l1_loss(
            velocity_error_scaled,
            torch.zeros_like(velocity_error_scaled),
            reduction="none",
            beta=float(config.huber_beta),
        )
    else:
        raise ValueError(f"Unknown velocity_loss_type: {config.velocity_loss_type}")
    velocity_loss = weighted_mean(velocity_per_axis, weight_batch)
    ...
    residual_error = pred_scaled - residual_batch
    if config.residual_loss_type == "mse":
        residual_per_axis = residual_error ** 2
    elif config.residual_loss_type == "smoothl1":
        residual_per_axis = F.smooth_l1_loss(
            pred_scaled,
            residual_batch,
            reduction="none",
            beta=float(config.huber_beta),
        ...
        raise ValueError(f"Unknown residual_loss_type: {config.residual_loss_type}")
    residual_loss = weighted_mean(residual_per_axis, weight_batch)
...
def main():
    ...
    parser.add_argument("--residual-loss-weight", type=float, default=0.10, help="Auxiliary residual-shape loss weight. V12 default is lower than V11T to let corrected-velocity loss dominate.")
    parser.add_argument("--gate-penalty", type=float, default=0.001, help="Small penalty on mean gate to prefer safe corrections unless useful. V12 default is slightly lower to reduce under-correction.")
    parser.add_argument("--velocity-loss", choices=["mse", "smoothl1"], default="smoothl1", help="Main corrected-vxy loss. V12 default smoothl1 is Huber-like and more robust than MSE.")
    parser.add_argument("--residual-loss", choices=["mse", "smoothl1"], default="smoothl1", help="Auxiliary residual-shape loss.")
    parser.add_argument("--huber-beta", type=float, default=1.0, help="SmoothL1 beta in scaled residual units. Smaller values become more L1-like.")
    parser.add_argument("--circle-loss-multiplier", type=float, default=2.0, help="Multiply training sample weights for maneuver_name == circle. Use 1.0 to disable.")
    ...
    print("Version 12 correction/evaluation:")
    ...
    print(f"  velocity_loss={args.velocity_loss}")
    print(f"  residual_loss={args.residual_loss}")
    print(f"  huber_beta={args.huber_beta}")
    print(f"  circle_loss_multiplier={args.circle_loss_multiplier}")
    ...
    if args.circle_loss_multiplier != 1.0:
        train_data = apply_maneuver_loss_multiplier(train_data, "circle", args.circle_loss_multiplier)
    ...
    if balanced_group_loss:
        print("\nTrain sample weights after balancing/maneuver multiplier:")
    ...
    loss_config = LossConfig(
        ...
        residual_loss_type=args.residual_loss,
        huber_beta=args.huber_beta,
    )

    best_val = float("inf")
    best_path = os.path.join(args.out_dir, "v12_smoothl1_circle_gated_avgmaxpool_best.pt")
    epochs_without_improvement = 0
    history = []

    print("\nTraining Version 12 SmoothL1/circle-weighted terrain+maneuver-aware learned-gate VXY residual model")
        ...
        if val_parts["loss"] < best_val - 1e-5:
            ...
            torch.save({
                ...
                "loss_config": {
                    ...
                    "huber_beta": args.huber_beta,
                    "circle_loss_multiplier": args.circle_loss_multiplier,
                },
            }, best_path)
        else:
    ...
    metrics = {
        ...
        "note": "Version 12 is V11T with SmoothL1/Huber velocity/residual losses and optional circle maneuver weighting. It predicts vx/vy residuals plus learned per-axis gates using terrain and maneuver context. wz is intentionally unchanged.",
    }
